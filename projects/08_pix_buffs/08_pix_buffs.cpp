...
void SetupRC()
{
	...
	// Create blur program
	blurProg = gltLoadShaderWithFileEx("blur.vs.glsl", "blur.fs.glsl", 2,
	...
	// 操作glReadPixels, glGetTexImage, and glGetCompressedTexImage在GPU中进行
	glBindBuffer(GL_PIXEL_PACK_BUFFER, pixBuffObjs[0]);
	glBufferData(GL_PIXEL_PACK_BUFFER, pixelDataSize, pixelData, GL_DYNAMIC_COPY);
	// 解除PBO绑定
	glBindBuffer(GL_PIXEL_PACK_BUFFER, 0);
...
void RenderScene(void)
{
	...
	if (isUsePBO) {
		...
		// 操作glTexImage*D, glTexSubImage*D, glCompressedTexImage*D, glCompressedTexSubImage*D 在GPU中进行
