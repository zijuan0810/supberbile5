...
*/
void gltPrintOpenGLInfo()
{
	printf("Vendor: \t%s\n", glGetString(GL_VENDOR));
	printf("Render: \t%s\n", glGetString(GL_RENDER));
	printf("OpenGL Version: \t%s\n", glGetString(GL_VERSION));
	printf("GLUT Version: \t%s\n", glGetString(GLU_VERSION));
}
				  ...
				  GLint iSlices, GLint iStacks)
{
	...
	double minorStep = 2.0f*M3D_PI / iStacks;

	torusBatch.BeginMesh(iSlices * (iStacks+1) * 6);
	for (int i=0; i<iSlices; ++i) {
		...
		for (int j=0; j<=iStacks; ++j) {
			...
			// First point
			vTexture[0][0] = (float)(i)/(float)(iSlices);
			vTexture[0][1] = (float)(j)/(float)(iStacks);
			vNormal[0][0] = x0*c;
			vNormal[0][1] = y0*c;
			...
			// Second point
			vTexture[1][0] = (float)(i+1)/(float)(iSlices);
			vTexture[1][1] = (float)(j)/(float)(iStacks);
			vNormal[1][0] = x1*c;
			vNormal[1][1] = y1*c;
			...
			vTexture[2][0] = (float)(i)/(float)(iSlices);
			vTexture[2][1] = (float)(j+1)/(float)(iStacks);
			vNormal[2][0] = x0*c;
			vNormal[2][1] = y0*c;
			...
			vTexture[3][0] = (float)(i+1)/(float)(iSlices);
			vTexture[3][1] = (float)(j+1)/(float)(iStacks);
			vNormal[3][0] = x1*c;
			vNormal[3][1] = y1*c;
				 ...
				 GLint iSlices, GLint iStacks)
{
	...
	fStepSizeRadial /= float(iStacks);

	GLfloat fStepSizeSlice = (3.1415926536f * 2.0f) / float(iSlices);

	diskBatch.BeginMesh(iSlices * iStacks * 6);
	...
	for(GLint i = 0; i < iStacks; i++)			// Stacks
	{
		float theyta;
		float theytaNext;
		for(GLint j = 0; j < iSlices; j++)     // Slices
		{
...
{	
	...
	cylinderBatch.BeginMesh(iSlices * iStacks * 6);
	...
	for (int i = 0; i < iStacks; i++) 
	{
		...
		float fCurrentZ = float(i) * (fLength / float(iStacks)); 
		float fNextZ = float(i+1) * (fLength / float(iStacks));
					 ...
					 GLenum *eFormat)
{
	...
	*iHeight = tgaHeader.height;
	...
	switch(sDepth) {
#ifndef OPENGL_ES
	case 3:     // 按照红、绿、蓝成分存储文理单元
		*eFormat = GL_BGR;
		*iComponents = GL_RGB;
		break;
#endif
	case 4:		// 按照红、绿、蓝和alpha成分存储文理单元
		*eFormat = GL_BGRA;
		*iComponents = GL_RGBA;
		break;
	case 1:		// 按照亮度值存储文理单元
...
GLbyte* gltReadBMPBits(const char *szFileName, int *nWidth, int *nHeight)
{
	...
	GLbyte *pBits = nullptr;					// Bitmaps bits
...
bool gltLoadShaderWithFile(const char *szFile, GLuint shader)
{
	...
	// Method 2: See how long the file is
	//fseek(fp, 0, SEEK_END);
	//long sz = ftell(fp);
	//fseek(fp, 0, SEEK_SET);
	...
	if (s_shaderText != nullptr) {
		fread(s_shaderText, 1, shaderLength, fp);
	}

	// Make sure it is null terminated and close the file
	s_shaderText[shaderLength] = '\0';
	fclose(fp);
...
GLuint gltLoadShaderWithFileEx(const char *szVertexProg, const char *szFragmentProg, ...)
{
	...
	GLuint hReturn = glCreateProgram();
...
void gltShowVersionInfo()
{
	...
	std::cout << "GLUT Version: " << gluGetString(GLU_VERSION) << std::endl;
}
...
bool gltLoadTextureTGA(const char* pszFileName, GLenum minFilter, GLenum magFilter, GLenum warpMode)
{
	...
	GLbyte* pBits = gltReadTGABits(pszFileName, &nWidth, &nHeight, &nComponents, &eFormat);
	if ( pBits == nullptr ) {
		return false;
	}
...
bool gltLoadTextureTGARect(const char* pszFileName, GLenum minFilter, GLenum magFilter, GLenum warpMode)
{
	...
	glTexParameteri(GL_TEXTURE_RECTANGLE, GL_TEXTURE_WRAP_S, warpMode);
	glTexParameteri(GL_TEXTURE_RECTANGLE, GL_TEXTURE_WRAP_T, warpMode);
