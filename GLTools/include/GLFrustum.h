...
public:
	...
	GLFrustum(GLfloat xMin, GLfloat xMax, GLfloat yMin, GLfloat yMax, GLfloat zMin, GLfloat zMax) { 
		this->SetOrthographic(xMin, xMax, yMin, yMax, zMin, zMax); 
	}
	 ...
	 */
	void SetOrthographic(GLfloat xMin, GLfloat xMax, GLfloat yMin, GLfloat yMax, GLfloat zMin, 
		GLfloat zMax) {
		m3dMakeOrthographicMatrix(_projMatrix, xMin, xMax, yMin, yMax, zMin, zMax);
		_projMatrix[15] = 1.0f;
	...
	void SetPerspective(float fFov, float fAspect, float fNear, float fFar) {
		...
		m3dLoadIdentity44(_projMatrix);
		_projMatrix[0] = (2.0f * fNear)/(xmax - xmin);
		_projMatrix[5] = (2.0f * fNear)/(ymax - ymin);
		_projMatrix[8] = (xmax + xmin) / (xmax - xmin);
		_projMatrix[9] = (ymax + ymin) / (ymax - ymin);
		_projMatrix[10] = -((fFar + fNear)/(fFar - fNear));
		_projMatrix[11] = -1.0f;
		_projMatrix[14] = -((2.0f * fFar * fNear)/(fFar - fNear));
		_projMatrix[15] = 0.0f;
	...
	bool TestSphere(float x, float y, float z, float fRadius) {
		...
		return this->TestSphere(vPoint, fRadius);
	}


	/**
	* Test a point against all frustum planes. A negative distance for any single plane means it 
	* is outside the frustum. The radius value allows to test for a point (radius = 0), or a 
	* sphere. Possibly there might be some gain in an alternative function that saves the addition 
	* of zero in this case.  Returns false if it is not in the frustum, true if it intersects
	* the Frustum.
	*/
